...
// 		$survey = $this->dom_doc->getElementById('survey');
		...
		$html .= '<button class="w3-btn w3-blue" type="submit">Absenden</button></form>';
		return $html;
	}
...
// 			var_dump($child_node->nodeName);
			...
			if($child_node->nodeName == '#text' && trim($child_node->textContent) == ''){
				...
				$html .= '<input class="w3-margin-bottom" type="radio" value="" name="'.$mother_name.'" '.$toggle_string.'>&nbsp;'.$child_node->nodeValue.'</input>&nbsp;<br>';

			}else if($child_node->nodeName == 'answer'){

				if($mother_type == 'textarea'){
					$html .= '<div class="w3-border-bottom"><textarea rows="8" cols="50" style="margin-bottom:32px">';
				}
